...
/**
 * Menu for the Fishing Block GUI.
 *
 * <p>Slot layout:
 * <pre>
 ...
 * <ul>
 *   <li>Index 0 — {@code fishTimer} (current countdown)
 *   <li>Index 1 — {@code fishInterval} (20 s; drives the progress bar)
 * </ul>
 */
public class FishingBlockMenu extends AbstractContainerMenu {

    @Nullable private final FishingBlockEntity blockEntity;
    ...
    public FishingBlockMenu(int containerId, Inventory inventory, FishingBlockEntity be) {
        super(VillageMod.FISHING_BLOCK_MENU.get(), containerId);
        this.blockEntity   = be;
    ...
    public FishingBlockMenu(int containerId, Inventory inventory, FriendlyByteBuf buf) {
        ...
        if (be instanceof FishingBlockEntity fishing) {
            this.blockEntity   = fishing;
            this.containerData = fishing.data;
        } else {
    ...
    private void addOutputSlots(@Nullable FishingBlockEntity be) {
    ...
    public boolean stillValid(Player player) {
        ...
        return player.distanceToSqr(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5) < 64.0
                && player.level().getBlockState(pos).is(VillageMod.FISHING_BLOCK.get());
    }
    ...
    public int     getFishTimer()    { return containerData.get(0); }
    public int     getFishInterval() { return containerData.get(1); }
}
