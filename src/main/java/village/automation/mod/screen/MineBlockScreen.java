 ...
 *   y = 93   mining progress bar  (160 px wide, 5 px tall)
 *   y = 99   divider
 ...
 * </pre>
 *
 * <p>The progress bar fills left-to-right as the mining timer counts down toward
 * the next drop.  Full = drop is about to be generated; empty = just reset.
 */
public class MineBlockScreen extends AbstractContainerScreen<MineBlockMenu> {
    ...
    private static final int COL_BAR_FG    = 0xFF5599FF;   // blue mining-progress fill

    // ── Constructor ───────────────────────────────────────────────────────────

    public MineBlockScreen(MineBlockMenu menu, Inventory inventory, Component title) {
    ...
    protected void renderBg(GuiGraphics g, float partialTick, int mx, int my) {
        ...
        // elapsed = MINE_INTERVAL - mineTimer; fills left-to-right as time passes
        int interval = menu.getMineInterval();
        int timer    = menu.getMineTimer();
        if (interval > 0) {
    ...
    /**
     * Draws in-panel labels.  The pose is already translated by (leftPos, topPos)
     * when this is called, so all coords here are panel-relative.
     * We do NOT call {@code super.renderLabels} to suppress the default title.
     */
    @Override
    ...
    public void render(GuiGraphics g, int mx, int my, float partialTick) {
        ...
        // Progress bar hover tooltip — shows seconds until next drop
        ...
        if (mx >= bx && mx < bx + BAR_W && my >= by && my < by + BAR_H) {
            ...
            if (interval > 0) {
                ...
                g.renderTooltip(this.font,
                        List.of(Component.literal("Next drop in: " + secsLeft + "s")),
